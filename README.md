<h1 align="center">السَّلاَمُ عَلَيْكُمْ وَرَحْمَةُ اللهِ وَبَرَكَاتُهُ</h1>

<h2 align="center">Hi, I'm Cak Gup</h2>

<p align="center">
  Cybersecurity & IT Governance Specialist · Ministry of Finance · Indonesia
</p>

<p align="center">
  <a href="https://cakgup.github.io/">
    <img src="https://img.shields.io/badge/Portfolio-cakgup.github.io-0A66C2?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Portfolio" />
  </a>
  <a href="https://orcid.org/0000-0003-2187-7808">
    <img src="https://img.shields.io/badge/ORCID-0000--0003--2187--7808-A6CE39?style=for-the-badge&logo=orcid&logoColor=white" alt="ORCID" />
  </a>
  <a href="https://gitlab.com/cakgup">
    <img src="https://img.shields.io/badge/GitLab-cakgup-FC6D26?style=for-the-badge&logo=gitlab&logoColor=white" alt="GitLab" />
  </a>
  <a href="https://www.linkedin.com/in/cakgup">
    <img src="https://img.shields.io/badge/LinkedIn-cakgup-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
</p>

---

## About Me

I am an IT Specialist and cybersecurity enthusiast with a strong interest in security governance, infrastructure protection, secure application practices, and digital public service improvement.

I currently contribute in a government treasury systems environment, where I focus on practical technology implementation, information security, IT governance, and operational reliability. I enjoy building lightweight tools that support security awareness, incident response, internal governance, civic-tech initiatives, and community-based digital services.

- **Current focus:** cybersecurity governance, penetration testing workflow, secure IT operations, and cloud security practices.
- **Professional interest:** security tooling, ITSM, infrastructure security, compliance, DevSecOps, and public sector digital transformation.
- **Academic background:** information security, computer science, and governance-oriented technology implementation.
- **Open to collaborate on:** security tools, governance frameworks, community platforms, civic-tech projects, and meaningful digital services.

---

## Tech Stack & Tools

<p>
  <img src="https://skillicons.dev/icons?i=js,ts,python,nodejs,react,nextjs,html,css,tailwind,git,github,docker,linux,bash,mysql,postgres,aws" alt="Tech Stack" />
</p>

### Specialization Areas

- Cybersecurity & Penetration Testing
- IT Governance, ITSM, and Compliance
- Infrastructure & Network Security
- Secure Application Development
- Cloud Architecture and Cloud Security
- Incident Response and Security Operations
- DevOps, CI/CD, and Application Quality Assurance
- Public Sector Digital Transformation

---

## Public Repositories

### Cybersecurity & Governance

- [panah](https://github.com/cakgup/panah) — Pentest Automation and Network Assessment Hub (PANAH).
- [satria](https://github.com/cakgup/satria) — Security Assessment, Tracking, Remediation, and Incident Automation (SATRIA).
- [redteam-console-WSL](https://github.com/cakgup/redteam-console) — Authorized Lab Emulation Console With WSL + Windows OS.
- [pentest-djpb](https://github.com/cakgup/pentest-djpb) — Structured guidance for authorized penetration testing workflow.
- [simple-pentest-tools](https://github.com/cakgup/simple-pentest-tools) — Lightweight browser-based toolkit for security testing support.
- [pentest-command-trainer](https://github.com/cakgup/pentest-command-trainer) — Typing trainer designed specifically for BSSN Pentester Cert.

### Civic-Tech, Community, and Public Service Tools

- [tahfidz](https://github.com/cakgup/tahfidz) — Tahfidz application for memorization, and monitoring of Al-Quran learning progress.
- [dashboard-masjid](https://github.com/cakgup/dashboard-masjid) — Digital information dashboard for mosque display.
- [ppsa](https://github.com/cakgup/ppsa)  — Digital doa and wirid application with offline support and prayer schedule.
- [ppsa_ms](https://github.com/cakgup/ppsa_ms)  — Pesantren management information system.
- [task](https://github.com/cakgup/task) — Family task management application with values-based design.
- [pagipetang](https://github.com/cakgup/pagipetang) — Static Islamic reading application for daily remembrance.
- [qurbanhub](https://github.com/cakgup/qurbanhub) — Web prototype for qurban program management.

### Web Utilities and Digital Presence

- [cakgup.github.io](https://github.com/cakgup/cakgup.github.io) — Personal portfolio website.
- [s](https://github.com/cakgup/s) — GitHub Pages, Google Apps Script, and Google Sheets based shortlink system.
- [u](https://github.com/cakgup/u) — Organizational microsite for Yayasan Indonesia Maju Gemilang.

### Cultural, Family, and Invitation Projects

- [bani-raukhan-rafi](https://github.com/cakgup/bani-raukhan-rafi) — Interactive family tree with cultural visual identity.
- [wedding-invitation](https://github.com/cakgup/wedding-invitation) — Google Sheets API based wedding invitation template.
- [invitation](https://github.com/cakgup/invitation) — Islamic modern digital invitation template.

---

## Connect with Me

<p>
  <a href="https://cakgup.github.io/">
    <img src="https://img.shields.io/badge/Website-000000?style=for-the-badge&logo=About.me&logoColor=white" alt="Website" />
  </a>
  <a href="https://www.linkedin.com/in/cakgup">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://orcid.org/0000-0003-2187-7808">
    <img src="https://img.shields.io/badge/ORCID-A6CE39?style=for-the-badge&logo=orcid" alt="ORCID" />
  </a>
</p>

## OpenPGP

<p align="center">
  <strong>Signed communication and identity verification</strong>
</p>

<p align="center">
  <code>ED71 8D3A F0C3 380E F19D 7AC8 4B88 21D8 E42B 927F</code>
</p>

<p align="center">
  Long fingerprint for public key verification.
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=cakgup&style=flat-square&color=0A66C2" alt="Profile views" />
</p>

---

<p align="center">
  <i>اِنْ تَنْصُرُوا اللّٰهَ يَنْصُرْكُمْ وَيُثَبِّتْ اَقْدَامَكُمْ</i>
</p>
